...
class MQTTManager: NSObject, CocoaMQTTDelegate, ObservableObject {
    ...
    private var cocoaMqttQos = CocoaMQTTQoS(rawValue: 1)!
...
//    }
    ...
    func connect() {
        ...
        let mqttQos  = UserData.shared.mqttQos
        let mqttKeepAlive = UInt16(UserData.shared.mqttKeepAlive)
        ...
        // Keep Alive 設定
        mqtt?.keepAlive = mqttKeepAlive
        
        // Qos
        cocoaMqttQos = CocoaMQTTQoS(rawValue: UInt8(mqttQos)) ?? .qos1
        print("cocoaMqttQos: \(cocoaMqttQos)")
    ...
    func publishBeaconData(beaconData: [[String: Any]], mqttTopic: String) {
        ...
        if let jsonData = try? JSONSerialization.data(withJSONObject: beaconData, options: []) {
            if let jsonString = String(data: jsonData, encoding: .utf8) {
                mqtt?.publish(mqttTopic, withString: jsonString, qos: cocoaMqttQos)
    ...
    func publish(topic: String, message: String) {
        ...
        startBackgroundTask()
        mqtt?.publish(topic, withString: message, qos: cocoaMqttQos)
