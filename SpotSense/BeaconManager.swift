...
class BeaconManager: NSObject, ObservableObject, CLLocationManagerDelegate , CocoaMQTTDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didRange beacons: [CLBeacon], satisfying constraint: CLBeaconIdentityConstraint) {
        ...
        DispatchQueue.main.async {
            ...
            // 1個のiBeaconが複数表示されることがあることへの対策
            let unique = Dictionary(grouping: beacons, by: {
                BeaconKey(major: $0.major.intValue, minor: $0.minor.intValue)
            })
            let filtered = unique.compactMap { $0.value.first }
            self.detectedBeacons = filtered // ContentView に反映
        }
        // 全ビーコンのRSSIをバッファに追加
    ...
    func handleEnterRegion() {
        ...
        var enterMsgStr = "enter"
        if wasRebooted || !hasLaunched {
            // enterメッセージにマークを付ける
            enterMsgStr = "enter*"
            // 通知とバッジ表示
            NotificationUtils.shared.scheduleBeaconNotification() // SpotSenseの起動をお願いする通知を表示
            UNUserNotificationCenter.current().setBadgeCount(1) // アプリアイコンにバッジ
        }
        ...
        DispatchQueue.global().asyncAfter(deadline: .now() + 5.0) {
            if self.isConnectedToMQTT {
                ...
                self.startRangingCycle()
                self.mqttManager.publish(topic: self.mqttTopic, message: "{\"id\": \"\(self.userId)\",\"event\": \"\(enterMsgStr)\"}")
    ...
    func logToFile(_ message: String) {
        guard UserData.shared.loggingFlg else {
            print("loggingFlg is false")
            return }
    ...
    private func retrieveUserData() -> Bool {
        ...
        if let storedMqttQos = UserDefaults.standard.string(forKey: "mqttQos") {
            UserData.shared.mqttQos = Int(storedMqttQos)!
        } else {
            return false
        }
        
        if let storedMqttKeepAlive = UserDefaults.standard.string(forKey: "mqttKeepAlive") {
            UserData.shared.mqttKeepAlive = Int(storedMqttKeepAlive)!
        } else {
            return false
        }
        ...
        if let logFlg = UserDefaults.standard.object(forKey: "loggingFlg") as? Bool {
            UserData.shared.loggingFlg = logFlg
        } else {
            return false
        }
        return true
