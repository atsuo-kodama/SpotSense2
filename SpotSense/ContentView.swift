...
struct ContentView: View {
    ...
    private var defaultMqttQos: String = "1"
    private var defaultMqttKeepAlive : String = "300"
        ...
        VStack (spacing: 1){
            ...
            HStack{
                ...
                Button(action: { // 設定ボタンが押された
                    ...
                    //Image(systemName: "gearshape")
                    Image(systemName: "restart").scaleEffect(x:-1, y:1)
                        .frame(width: 25, height: 25)
                }
                .padding()
                .fullScreenCover(isPresented: $isPresented) {
    ...
    private func retrieveUserData() -> Bool {
        ...
        if let storedMqttQos = UserDefaults.standard.string(forKey: "mqttQos") {
            userData.mqttQos = Int(storedMqttQos)!
        } else {
            UserDefaults.standard.set(defaultMqttQos, forKey: "mqttQos")
            userData.mqttQos = Int(defaultMqttQos)!
        }
        
        if let storedMqttKeepAlive = UserDefaults.standard.string(forKey: "mqttKeepAlive") {
            userData.mqttKeepAlive = Int(storedMqttKeepAlive)!
        } else {
            UserDefaults.standard.set(defaultMqttKeepAlive, forKey: "mqttKeepAlive")
            userData.mqttKeepAlive = Int(defaultMqttKeepAlive)!
        }
...
class UserData: ObservableObject {
    ...
    @Published var mqttQos: Int = 0
    @Published var mqttKeepAlive: Int = 0
